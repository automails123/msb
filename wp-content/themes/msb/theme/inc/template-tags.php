...
if ( ! function_exists( 'msb_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function msb_posted_on() {
...
if ( ! function_exists( 'msb_posted_by' ) ) :
	/**
	 * Prints HTML with meta information about theme author.
	 */
	function msb_posted_by() {
		printf(
		/* translators: 1: posted by label, only visible to screen readers. 2: author link. 3: post author. */
			'<span class="sr-only">%1$s</span><span class="author vcard"><a class="url fn n" href="%2$s">%3$s</a></span>',
			esc_html__( 'Posted by', 'msb' ),
...
if ( ! function_exists( 'msb_comment_count' ) ) :
	/**
	 * Prints HTML with the comment count for the current post.
	 */
	function msb_comment_count() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			/* translators: %s: Name of current post. Only visible to screen readers. */
			comments_popup_link( sprintf( __( 'Leave a comment<span class="sr-only"> on %s</span>', 'msb' ), get_the_title() ) );
		}
	}
endif;

if ( ! function_exists( 'msb_entry_meta' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 * This template tag is used in the entry header.
	 */
	function msb_entry_meta() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( __( ', ', 'msb' ) );
			if ( $categories_list ) {
				printf(
				/* translators: 1: posted in label, only visible to screen readers. 2: list of categories. */
					'<span><span class="sr-only">%1$s</span>%2$s</span>',
					esc_html__( 'Posted in', 'msb' ),
					$categories_list // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				);
			}

			/* translators: used between list items, there is a space after the comma. */
			$tags_list = get_the_tag_list( '', __( ', ', 'msb' ) );
			if ( $tags_list ) {
				printf(
		...
		edit_post_link(
			sprintf(
				wp_kses(
				/* translators: %s: Name of current post. Only visible to screen readers. */
					__( 'Edit <span class="sr-only">%s</span>', 'msb' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'msb_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function msb_entry_footer() {
...
if ( ! function_exists( 'msb_post_thumbnail' ) ) :
	...
	function msb_post_thumbnail() {
		if ( ! msb_can_show_post_thumbnail() ) {
			return;
		}
...
if ( ! function_exists( 'msb_comment_avatar' ) ) :
	/**
	 ...
	 */
	function msb_get_user_avatar_markup( $id_or_email = null ) {

		if ( ! isset( $id_or_email ) ) {
			$id_or_email = get_current_user_id();
		}

		return sprintf( '<div class="vcard">%s</div>', get_avatar( $id_or_email, msb_get_avatar_size() ) );
	}
endif;

if ( ! function_exists( 'msb_discussion_avatars_list' ) ) :
	/**
	 * Displays a list of avatars involved in a discussion for a given post.
	 *
	 * @param array $comment_authors Comment authors to list as avatars.
	 */
	function msb_discussion_avatars_list( $comment_authors ) {
		...
		foreach ( $comment_authors as $id_or_email ) {
			printf(
				"<li>%s</li>\n",
				msb_get_user_avatar_markup( $id_or_email ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			);
		}
		echo '</ol>', "\n";
	}
...
if ( ! function_exists( 'msb_the_posts_navigation' ) ) :
	/**
	 * Wraps `the_posts_pagination` for use throughout the theme.
	 */
	function msb_the_posts_navigation() {
		the_posts_pagination(
			array(
				'mid_size'  => 2,
				'prev_text' => __( 'Newer posts', 'msb' ),
				'next_text' => __( 'Older posts', 'msb' ),
			)
		);
	}
endif;

if ( ! function_exists( 'msb_content_class' ) ) :
	/**
	 ...
	 * MSB_TYPOGRAPHY_CLASSES constant.)
	 *
	 ...
	 */
	function msb_content_class( $classes = '' ) {
		$all_classes = array( $classes, MSB_TYPOGRAPHY_CLASSES );
